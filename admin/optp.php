...
<?php 
/*小安微贴吧 主题操作页 开发语言：PHP 数据库：MYSQL 官方网站http://www.xiaoan.gq/
...
if($action=="del")
{
    ...
    $rs = mysql_query($sql);
    if(!$rs){die("错误：无法连接到数据库!(1)<a href='topic.php'>返回</a>");}
	$sql = "DELETE FROM wtb_reply WHERE tid='$tid'";
    $rs = mysql_query($sql);
    if(!$rs){die("错误：无法连接到数据库!(2)<a href='topic.php'>返回</a>");}
	echo "<b><font color='green'>删贴成功!</font></b><a href='topic.php'>返回</a>";
	exit;
}
elseif($action=="top")
{
